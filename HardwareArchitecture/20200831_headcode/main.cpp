...
Motor top_motor(PF_12,PF_11,PA_1);
Motor neck_motor(PG_1,PG_0,PA_2); // PE_12,PE_11,PB_15
Motor left_motor(PE_8,PE_9,PB_14);   // PG_1,PG_0,PA_2
Motor right_motor(PE_12,PE_11,PB_15);  //PE_9,PE_8,PB_14
...
int main(int, const char * const [])
{
  ...
  while (1)
  {
    ...
    // --------- top motor control --------//
      ...
      float top_drive = pid_controller(0.5,0.001,0.5,set_top_ang,head_top_ang,pre_error[0],i_term[0],d_term[0]);
      top_motor.drive(-0.3);//1.65,0.0001,0.3  top_drive/100.00
      // top = top_drive;

      // --------- left motor control --------//1

      if(set_left_ang > head_left_ang ){ // ก้ม 0.6,0.01,1.3 +  1.0 0.003 4.0
        float left_drive = pid_controller(0.5,0.0,0.0,set_left_ang,head_left_ang,pre_error[1],i_term[1],d_term[1]);
        // left_motor.drive(left_drive/100.00); //1.4,0.01,3.0
    ...
    ///// --------- right motor control --------/////////
      if(set_right_ang > head_right_ang ){ // ก้ม 0.6,0.01,1.3 +
        float right_drive = pid_controller(0.5,0.0,0.0,set_right_ang,head_right_ang,pre_error[2],i_term[2],d_term[2]);
