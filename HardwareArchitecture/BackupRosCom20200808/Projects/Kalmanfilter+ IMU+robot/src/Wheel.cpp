...
void Driver::setWheel(int position,PinName pwmf,PinName pwmb) {
    wheel[position] = new Wheel(pwmf,pwmb);
}
void Driver::drive(float Vx,float Vy,float Wz,float speed_satuated){
    ...
    // float value[4];
    value[0] =((value[0]*19)+(Vx - Vy + (Wz*(-0.77))))/20;  // 0.28 sum xy จากหลางล้อ ถึง origin
    value[1] =((value[1]*19)+(Vx + Vy + (Wz*(0.77))))/20;  // รถใหม่ 0.77472 m. moving average19/20
    value[2] =((value[2]*19)+(Vx + Vy + (Wz*(-0.77))))/20; 
    value[3] =((value[3]*19)+(Vx - Vy + (Wz*(0.77))))/20;
